...
func (c connectorsRepository) FindOne(connectorModel *model.Connector) (model.Connector, error) {
	...
	err := c.collection.FindOne(context.TODO(), connectorModel).Decode(&connector)
...
func (c connectorsRepository) Update(connector *model.Connector) error {
	var _, err = c.collection.UpdateOne(context.TODO(), model.Connector{ConnectorID: connector.ConnectorID, ChargerID: connector.ChargerID}, connector)
	if err == mongo.ErrNoDocuments {
		return err
	}else if err != nil {
		panic(err)
